...
def auto_configure(n_features: int, seq_len: int, n_train: int = None):
    ...
    # Override with sample count if very large
    if n_train is not None and n_train > 15000 and tier != "large":
        tier = "large" if n_features > 12 else "medium"
...
class KePINModel(keras.Model):
    ...
    def __init__(self, input_shape_tuple, arch_config=None, n_train=None, **kwargs):
        ...
        self.head_dense1 = keras.layers.Dense(
            64, activation="relu", kernel_initializer="he_normal", name="head_dense1",
        )
        self.head_dropout = keras.layers.Dropout(
            arch_config["dropout"], name="head_dropout",
        )
        # Output layer in float32 for numerical stability under mixed precision
    ...
    def call(self, inputs, training=None):
        ...
        for block in self.encoder_blocks:
            ...
            x = block["relu"](x)
            # SE attention is applied inline
            x = se_block_1d(x, ratio=8)
        ...
        pool_avg = self.gap(z_seq)             # (batch, d)
        pool_max = self.gmp(z_seq)             # (batch, d)
        pooled = self.concat([pool_avg, pool_max])  # (batch, 2d)
