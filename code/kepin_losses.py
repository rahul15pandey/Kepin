...
def rul_mse_loss(y_true, y_pred):
    ...
    return K.mean(K.square(K.flatten(y_true) - K.flatten(y_pred)))
...
def monotonicity_loss(y_true, y_pred):
    ...
    denom = K.sum(same_engine_mask) + K.epsilon()

    # Penalise positive predicted diffs (RUL increasing)
    mono_violation = K.relu(pred_diffs) * same_engine_mask
    return K.sum(mono_violation) / denom
...
def asymmetric_loss(y_true, y_pred):
    ...
    y_true_flat = K.flatten(y_true)
    y_pred_flat = K.flatten(y_pred)
    over = K.relu(y_pred_flat - y_true_flat)
    return K.mean(K.square(over))
...
def slope_matching_loss(y_true, y_pred):
    ...
    slope_err = K.square((pred_diffs - true_diffs) * same_engine_mask)
    return K.sum(slope_err) / denom
...
class KePINLossWeights(keras.layers.Layer):
    ...
    def call(self, loss_values):
        ...
        for i, loss_val in enumerate(loss_values):
            precision = tf.exp(-self.log_vars[i])      # 1/(2σ²)
            total = total + 0.5 * precision * loss_val + 0.5 * self.log_vars[i]
        weights = tf.exp(-self.log_vars)  # effective weights for monitoring
